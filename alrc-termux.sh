...
function al_log () {
cat $ALRC_HOME_alt/Changelog.al.txt
}

function alcat () {
  cat $ALRC_HOME_alt/${NAME:+alrc-termux.sh}
}
...
# 19 juni
alias al_enable_alcat='echo '_alcat' >> ~/.bash_history && source $ALRC_HOME_alt/$NAME'
alias al_enable_brandomusicv='source $ALRC_HOME_alt/lib/alrc-brandomusic_v.sh'
...
cat <<- EOF > $ALRC_HOME_alt/answer.txt
${answer}
EOF
...
mapfile -d '' content < $ALRC_HOME_alt/answer.txt
content="${content[*]%$'\n'}"
read -rd '' content <  $ALRC_HOME_alt/answer.txt
